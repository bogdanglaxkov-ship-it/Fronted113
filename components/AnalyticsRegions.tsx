...
export default function AnalyticsRegions() {
  ...
  const activeRegions = aggregated.filter((a) => a.count > 0).length;
  const totalBudget = aggregated.reduce((sum, a) => sum + a.totalBudget, 0);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <span className="mt-1 block font-mono text-lg text-primary">
          ...
          {aggregated.map((r) => {
            ...
            return (
              ...
              >
                ...
                {r.totalBudget > 0 && (
                  <p className="font-mono text-[10px] text-primary/80">{fmtKzt(r.totalBudget)}</p>
                )}
              </button>
            );
          })}
        </div>
        <div className="mt-4 flex gap-4 border-t border-border pt-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                ...
                {ALMATY_DISTRICTS.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="font-mono text-[10px] text-muted-foreground">{c} тендеров</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {Array.from(selectedAgg.districts.entries())
                  ...
                  .map(([d, c]) => (
                    ...
                    >
                      {d} <span className="font-mono text-primary">· {c}</span>
                    </span>
                  ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {selectedAgg.tenders.slice(0, 20).map((t) => (
                  <a
                    ...
                    className="block rounded-md border border-border bg-background p-3 transition-colors hover:border-primary hover:bg-accent/40"
                  >
